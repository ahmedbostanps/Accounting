...
<tr class="product_row" data-row_index="{{$row_count}}" @if(!empty($so_line)) data-so_id="{{$so_line->transaction_id}}" @endif>
	...
	<td>
		...
		<input type="hidden" value="{{$product->sub_sku}}"
			   name="products[{{$row_count}}][sub_sku]" class="row_weighing_scale_barcode_id">
...
{{--			   {{@format_quantity($product->quantity_ordered)}}--}}
			value="{{ round($product->quantity_ordered , 2) }}" name="products[{{$row_count}}][quantity]" data-allow-overselling="@if(empty($pos_settings['allow_overselling'])){{'false'}}@else{{'true'}}@endif"
			@if($allow_decimal) 
				data-decimal=1 
			@else 
				data-decimal=0 
			...
			@endif
				@if($product->unit_id == 2)	data-step="0.1" @else data-step="1" @endif
		...
		<input type="hidden" name="products[{{$row_count}}][product_unit_id]" class="product_unit_id" value="{{$product->unit_id}}">
	...
	<td class="text-center">
		<input type="text" class="form-control unit_price_inc" disabled value="{{@num_format($unit_price_inc_tax )}}">
