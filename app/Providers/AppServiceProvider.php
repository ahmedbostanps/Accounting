...
class AppServiceProvider extends ServiceProvider
{
    ...
    public function boot()
    {
        ...
        $productsAlert = Product::join('variations', 'products.id', '=', 'variations.product_id')
            ->active()
            ...
            ->leftjoin(
                ...
                function ($join) {
                    ...
                    $join->orWhereNull('VLD.location_id');
                }
            )->select(
            ...
            )->whereNotNull('VLD.qty_available')->whereRaw('VLD.qty_available <= products.alert_quantity')->get();
        foreach ($productsAlert as $row_alert) {
            $oldNoti = Notifications::where('type' , $row_alert->product_id)->where('notifiable_type' , 1)->whereNotNull('read_at')->first();
            if(!empty($oldNoti)) {
                $oldNoti->delete();
            }
//            if($row_alert->qty_available <= $row_alert->alert_quantity) {
//                dd(true , $row_alert->product_id);
                $oldNotiCheck = Notifications::where('type' , $row_alert->product_id)->where('notifiable_type' , 1)->whereNull('read_at')->first();
                if(empty($oldNotiCheck)) {
                    $notify = new Notifications();
                    $notify->type = $row_alert->product_id;
                    $notify->notifiable_type = 1;
                    $notify->notifiable_id = 1;
                    $notify->data = ' كمية المنتج التالي <span style="color: #f00">'.$row_alert->name.'</span> قد تعدت الكمية المسموحة بها ';
                    $notify->read_at = null;
                    $notify->save();
                }
